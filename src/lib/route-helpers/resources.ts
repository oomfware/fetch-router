...
import { createRoutes, type BuildRouteMap } from '../route-map.ts';
...
export function createResourcesRoutes<base extends string, const options extends ResourcesOptions>(
	...
	const newName = options?.names?.new ?? 'new';
	const showName = options?.names?.show ?? 'show';
	const createName = options?.names?.create ?? 'create';
	const editName = options?.names?.edit ?? 'edit';
	const updateName = options?.names?.update ?? 'update';
	const destroyName = options?.names?.destroy ?? 'destroy';
